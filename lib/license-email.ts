...
export async function sendLicenseEmail(record: LicenseRecord) {
  ...
  return resend.emails.send({
    ...
    subject: `Your Wappkit license for ${record.productName}`,
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827;">
        <h1 style="font-size: 20px;">Your Wappkit license</h1>
        <p>Here is a copy of the license for <strong>${record.productName}</strong>.</p>
        <ul>${licenseList}</ul>
        <p>Order ID: <strong>${record.orderId}</strong></p>
        <p>If you did not request this email, you can ignore it.</p>
      </div>
    `,
    text: `Your Wappkit license for ${record.productName}\n\n${textList}\n\nOrder ID: ${record.orderId}`,
  });
}
